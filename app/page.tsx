...
export default async function HomePage() {
  ...
  // Always fetch from Supabase; if not configured, return empty arrays to keep UI
  const banners = supabase
    ? ((await supabase.from("banners").select("*").order("created_at", { ascending: false }).limit(5)).data ?? [])
    : []

  const products: Product[] = supabase
    ? ((await supabase.from("products").select("*").order("created_at", { ascending: false }).limit(10)).data ?? [])
    : []
